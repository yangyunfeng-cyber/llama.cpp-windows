    ...
    (state.assistantDialog.type === 'list' ? (
      ...
      (state.assistants.length > 0 ? state.assistants.map(function(a) {
        return '<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--line)">' +
          '<div><strong style="font-size:13px">' + escapeHtml(a.name) + '</strong><p style="margin:2px 0 0;font-size:11px;color:var(--muted)">' + escapeHtml((a.systemPrompt || '').slice(0, 60)) + '</p></div>' +
...
function doClearContext() {
  state.chatMessages.push({ role: 'system', content: '─── 上下文已清除 ───', createdAt: Date.now(), localOnly: true })
  state.chatInput = ''
  state.attachments = []
...
function startFreshSession() {
  ...
  if (prompt) state.chatMessages.push({ role: 'system', content: prompt, createdAt: Date.now() })
}
...
function renderSidebar() {
  ...
  return `
    <aside class="sidebar">
      ...
      <div class="assistant-bar">${state.assistants.length > 0 ? state.assistants.map(function(a) { return '<button type="button" class="assistant-chip ' + (state.activeAssistantId === a.id ? 'active' : '') + '" data-action="switch-assistant" data-assistant-id="' + escapeAttribute(a.id) + '" title="' + escapeAttribute(a.systemPrompt || '') + '">' + escapeHtml(a.name) + '</button>' }).join('') : ''}<button type="button" class="outline-btn small-btn" data-action="manage-assistants" style="font-size:11px;font-weight:700">添加助手</button></div>
...
appEl.addEventListener('click', event => {
  ...
  if (action === 'switch-assistant' && target.dataset.assistantId) {
    state.activeAssistantId = state.activeAssistantId === target.dataset.assistantId ? '' : target.dataset.assistantId
    persistAssistants()
    if (state.assistantDialog && state.assistantDialog.type === 'list') {
      state.assistantDialog = null
    }
  ...
  if (action === 'save-assistant') {
    ...
    if (existingId) {
      ...
      if (asst) { asst.name = name; asst.systemPrompt = prompt }
    } else {
