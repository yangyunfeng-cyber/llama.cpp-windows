...
- 支持 **Ctrl+V** 从文件管理器粘贴文件、从截图工具粘贴图片为附件
...
- 🔤 **Token 显示国际化** — "个代币"改为 "tokens"，"t/s" 改为 "token/s"，符合通用技术惯例
- 📁 **文件类型扩展** — 新增 28 种文本/代码文件格式支持：bat、cmd、sh、ps1、xml、ini、go、java、swift、vue 等
- 📋 **粘贴附件功能** — 支持从文件管理器 Ctrl+V 粘贴文件，从截图工具 Ctrl+V 粘贴图片为附件
- 🔔 **退出确认对话框** — 点击窗口 × 时弹出三选一对话框：最小化到托盘 / 完全退出 / 取消
- 📤 **对话导出为 Markdown 文件** — 历史对话导出改为保存为 .md 文件，弹出保存路径选择框
- 🎨 **助手下拉控件重做** — 自定义下拉框替代原生 select，长名称不再挤压按钮，支持换行显示
- ⏪ **日志增量发送** — 优化 IPC 带宽，日志改为增量推送
